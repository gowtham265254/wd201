...
describe("Todo Application", function () {
    ...
    try {
      ...
      describe("Todo Application", function () {
        ...
        test("Deletes a todo with the given ID if it exists and sends a boolean response", async () => {
          ...
          const deleteTodoResponses = await agent
            .delete(`/todos/${todoID}`)
            .send();
  ...
  test("Deletes a todo with the given ID if it exists and sends a boolean response", async () => {
    ...
    const deleteNonExistentTodoResponss = await agent
      .delete(`/todos/9999`)
      .send();
    const parsedDeleteNonExistentTodoResponss = JSON.parse(
      deleteNonExistentTodoResponss.text
    );
    expect(parsedDeleteNonExistentTodoResponss).toBe(false);
  });
});
